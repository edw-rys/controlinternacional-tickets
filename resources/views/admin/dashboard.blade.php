...
@section('content')
	...
	<div class="page-header d-xl-flex d-block">
		<div class="page-leftheader">
			@switch($status)
				@case('active')
					<h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{ trans('langconvert.adminmenu.activetickets')}}</span></h4>
					@break
				@case('closed')
					<h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{ trans('langconvert.adminmenu.closedtickets')}}</span></h4>
					@break
				@case('closed')
					<h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{ trans('langconvert.adminmenu.onholdtickets')}}</span></h4>
					@break
				@case('overdue')
					<h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{ trans('langconvert.adminmenu.overduetickets')}}</span></h4>
					@break
								...
								<div class="row">
									<div class="col-8">
										<div class="mt-0 text-start"><span class="fs-14 font-weight-semibold">{{trans('langconvert.adminmenu.alltickets')}}</span>
											<h3 class="mb-0 mt-1 mb-2">{{$tickets}}</h3>
										</div>
									</div>
									<div class="col-4">
	...
	<script type="text/javascript">
		...
		(function($)  {
			...
			var table = $('#supportticket-dashe').DataTable({language : languajeDT,
				...
				ajax: {
					@if($status == 'all')
					url: "{{ route('admin.tickets-list', $status) }}"
					// url: "{{ url('admin') }}",
					@else
					url: "{{ route('admin.tickets-list', $status) }}"
					@endif
			...
			function loadDataTable() {
				...
				$('#supportticket-dashe').on('preXhr.dt', function (e, settings, data) {
