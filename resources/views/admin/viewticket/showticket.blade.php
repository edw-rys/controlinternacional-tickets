					...
					<div class="card">
						<div class="card-header border-0 mb-1 d-block">
							<div class="d-sm-flex d-block">
								...
								@php
									$status = trans('langconvert.admindashboard.' . (strtolower($ticket->status)));
								@endphp
								<div class="card-options float-sm-end ticket-status">
									...
									<span class="badge badge-teal">{{ $status }}</span>
							...
							<div class="table-responsive tr-lastchild">
								<table class="table mb-0 table-information">
									<tbody>
										...
										<tr>
											<td>
												<span class="w-50">{{trans('langconvert.admindashboard.tickethose')}}</span>
											</td>
											<td>:</td>
											<td>
												<span class="font-weight-semibold">{{ $ticket->hose != null ? $ticket->hose->name : '-'}}</span>
												@if ( $ticket->hose != null)	
												{{-- <tr>
													<td> --}}
														<br>
														<span class="font-weight-semibold">{{ $ticket->hose->hose_type != null ? $ticket->hose->hose_type->name : '-'}}</span>
													{{-- </td>
												</tr> --}}
												@endif
											</td>
										...
										<tr>
											...
											<td>
												...
												<span class="badge badge-success">{{ trans('langconvert.admindashboard.' . (strtolower($ticket->status))) }}</span>
												@elseif($ticket->status == "Re-Open")

												<span class="badge badge-teal">{{ trans('langconvert.admindashboard.' . (strtolower($ticket->status))) }}</span>
												@elseif($ticket->status == "Inprogress")

												<span class="badge badge-info">{{ trans('langconvert.admindashboard.' . (strtolower($ticket->status))) }}</span>
												@elseif($ticket->status == "On-Hold")

												<span class="badge badge-warning">{{ trans('langconvert.admindashboard.' . (strtolower($ticket->status))) }}</span>
												@else

												<span class="badge badge-danger">{{ trans('langconvert.admindashboard.' . (strtolower($ticket->status))) }}</span>
												@endif
	...
	<script type="text/javascript">
		...
		@endif
			...
			$('body').on('submit', '#priority_form', function (e) {
				...
				$('#pribtnsave').html('{{ trans('langconvert.admindashboard.savechanges')}}');
				location.reload();
				toastr.success(data.success);
					...
					<div class="modal-body">
						...
						<div class="custom-controls-stacked d-md-flex" >
							<select class="form-control select2_modalpriority" data-placeholder="Select Priority" name="priority_user_id" id="priority" >
								...
								<option value="Critical" {{($ticket->priority == 'Critical')? 'selected' :'' }}>{{trans('langconvert.newwordslang.critical' )}}</option>
								<option value="High" {{($ticket->priority == 'High')? 'selected' :'' }}>{{trans('langconvert.newwordslang.high' )}}</option>
								<option value="Medium" {{($ticket->priority == 'Medium')? 'selected' :'' }}>{{trans('langconvert.newwordslang.medium' )}}</option>
								<option value="Low" {{($ticket->priority == 'Low')? 'selected' :'' }}>{{trans('langconvert.newwordslang.low' )}}</option>
							</select>	
						</div>
