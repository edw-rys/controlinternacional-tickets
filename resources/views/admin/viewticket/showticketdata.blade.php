		...
		@else
			...
			<div class="card-body">
				<div class="d-sm-flex">
					...
					<div class="media-body">
						<h5 class="mt-1 mb-1 font-weight-semibold">{{ $comment->cust->username }}<span class="badge badge-danger-light badge-md ms-2">{{ trans('langconvert.admindashboard.' . strtolower($comment->cust->userType)) }}</span></h5>
						<small class="text-muted"><i class="feather feather-clock"></i> {{ $comment->created_at->diffForHumans() }}</small>
						<div class="fs-13 mb-0 mt-1">
							{!! $comment->comment !!}
