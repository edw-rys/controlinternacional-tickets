							...
							<form method="POST" id="user_form" enctype="multipart/form-data">
								...
								<div class="card-body">
										...
										<div class="row">
											...
											<div class="col-md-9">
												<select
													...
													data-placeholder="Selecciona Categoría" name="category" id="category">
									...
									<div class="form-group">
										<div class="row">
											<div class="col-md-3">
												<label class="form-label mb-0 mt-2">{{trans('langconvert.admindashboard.tickethose')}}<span class="text-red">*</span></label>
											</div>
											<div class="col-md-9">
												<select  class="form-control select2-show-search  select2 @error('hose') is-invalid @enderror"  data-placeholder="Seleccionar manguera" name="hose" id="hose">
													<option label="Seleccionar manguera"></option>
												</select>
												<span id="ChoseError" class="text-danger alert-message"></span>
												@error('hose')
													<span class="invalid-feedback" role="alert">
														<strong>{{ $message }}</strong>
													</span>
...
<script type="text/javascript">
	...
	@endif
	function loadHoses() {
		let url = '{{ route('client.get-hoses')}}'
		$.ajax({
			url: url,
			type: "GET",
			dataType: "json",
			success:function(data) {
				$('select[name="hose"]').empty();
				var html = '';
				for (const hose of data.data) {
					html+=`<option value="${hose.id}">${hose.name}</option>`
				}
				$('#hose').html(html);
			}
		});
