...
@section('content')
    ...
    <div class="pb-3 px-5 pt-3">
		<h1 class="mb-2" style="font-size: 1.5em">{{trans('langconvert.menu.login')}} - {{ $title_client}}</h1>
    ...
    @if ($socialAuthSettings->envato_status == 'enable' ||
        ...
        $socialAuthSettings->facebook_status == 'enable' ||
        $socialAuthSettings->twitter_status == 'enable')
        <div class="login-icons card-body border br-7 mx-5 my-3 p-4 d-flex align-items-center justify-content-center">
            @if ($socialAuthSettings->envato_status == 'enable')
                <a href="javascript:;" class="social-icon brround me-3 bg-primary-transparent text-primary"
                    data-bs-toggle="tooltip" title="Login with Envato" onclick="window.location.href = envato;"
            ...
            @if ($socialAuthSettings->google_status == 'enable')
                <a href="javascript:;" class="social-icon brround me-3 bg-primary-transparent text-primary"
                    data-bs-toggle="tooltip" title="Login with Google" onclick="window.location.href = google;"
                    data-bs-original-title="Login with Google">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 47 48">
            ...
            @if ($socialAuthSettings->facebook_status == 'enable')
                <a href="javascript:;" class="social-icon brround me-3 bg-primary-transparent text-primary"
                    data-bs-toggle="tooltip" title="Login with Facebook" onclick="window.location.href = facebook;"
                    data-bs-original-title="Login with Facebook">
                    <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">
                        <g data-name="facebook circle" transform="translate(-126.445 -2.281)">
                            <circle cx="50" cy="50" r="50" fill="#3c5a9a" data-name="Ellipse 2"
                                transform="translate(126.445 2.281)" />
                            <path fill="#fff"
                                d="M357.464,92.677H346.388c-6.573,0-13.884,2.765-13.884,12.292.032,3.32,0,6.5,0,10.078h-7.6v12.1h7.839v34.835h14.405V126.917h9.508l.86-11.9H346.9s.024-5.3,0-6.833c0-3.765,3.918-3.55,4.153-3.55,1.864,0,5.489.005,6.42,0V92.677Z"
                                data-name="Path 21" transform="translate(-164.761 -75.049)" />
                        </g>
                    </svg>
                </a>
            @endif
            @if ($socialAuthSettings->twitter_status == 'enable')
                <a href="javascript:;" class="social-icon brround me-3 bg-primary-transparent text-primary"
                    data-bs-toggle="tooltip" title="Login with Google" onclick="window.location.href = twitter;"
    ...
    <form class="card-body border-top-0 pt-3" id="login" action="{{ route('client.do_login') }}" method="post">
        ...
        <div class="form-group">
            <label class="form-label">{{ trans('langconvert.admindashboard.email') }} <span
        ...
        <div class="form-group">
            <label class="form-label">{{ trans('langconvert.admindashboard.password') }} <span
                    class="text-red">*</span></label>
        ...
        <div class="text-center mt-3">
            <a href="{{ url('customer/forgotpassword') }}"
                class="text-primary pb-2">{{ trans('langconvert.menu.forgotpass') }}</a>
            ...
            {{-- <p class="text-dark mb-0"><a class="text-primary ms-1"
                    href="{{ url('/') }}">{{ trans('langconvert.menu.meback') }}</a></p> --}}
        </div>
    </form>
    <!--Login Page-->
