    ...
    <form class="card-body pt-3" method="POST" action="{{ url('customer/reset-password') }}">
        ...
        <div class="form-group">
            ...
            <input id="email" type="email" class="form-control @error('email') is-invalid @enderror" name="email"
                value="{{ $user->email ?? old('email') }}" autocomplete="email" placeholder="john@gmail.com" autofocus
                readonly>
        ...
        <div class="form-group">
            <label class="form-label"
                for="password-confirm">{{ trans('langconvert.admindashboard.confirmpassword') }}</label>
            <input class="form-control" placeholder="password" id="password-confirm" name="password_confirmation"
                type="password">
        </div>
        <div class="submit">
