...
class AdminDashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // If no there in group we get the all tickets
            ...
            $active = Ticket::whereIn('status', ['New', 'Re-Open','Inprogress']);
            $closed = Ticket::where('status', 'Closed');
            $onhold = Ticket::where('status', 'On-Hold');
            $data['onhold'] = $onhold;
            $myticket = Ticket::where('user_id', auth()->id());
            $data['myticket'] = $myticket;
            $assigned = Ticket::where('myassignuser_id', Auth::id());
            $data['assigned'] = $assigned;
            $myassigned = Ticket::where('toassignuser_id', Auth::id());
            $data['myassigned'] = $myassigned;
            $overdue = Ticket::whereIn('overduestatus', ['Overdue']);
            $data['overdue'] = $overdue;
            ...
            if(request()->ajax()) {
                ...
                switch ($status) {
                    ...
                    case 'onhold':
                        ...
                        break;
                    case 'overdue':
                        $data->whereIn('overduestatus', ['Overdue']);
                        break;
                    default:
                        break;
                }
